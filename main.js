...
function updateCharts(data) {
  ...
  // Status chart with percentages (DONUT CHART)
  const statusCounts = {};
  data.forEach(r => {
            ...
            generateLabels: function(chart) {
              ...
              if (data.labels.length && data.datasets.length) {
                return data.labels.map((label, i) => {
                  ...
                  const percentage = ((count / totalStatus) * 100).toFixed(1);
                  return {
  ...
  // Education chart with percentages (PIE CHART)
  ...
  if (Object.keys(educationCounts).length > 0) {
    const totalEducation = Object.values(educationCounts).reduce((sum, count) => sum + count, 0);
          ...
          labels: { 
            ...
            generateLabels: function(chart) {
              ...
              if (data.labels.length && data.datasets.length) {
                return data.labels.map((label, i) => {
                  const count = data.datasets[0].data[i];
                  const percentage = ((count / totalEducation) * 100).toFixed(1);
                  return {
                    text: `${label}: ${count} (${percentage}%)`,
                    fillStyle: data.datasets[0].backgroundColor[i],
                    strokeStyle: data.datasets[0].backgroundColor[i],
        ...
        tooltip: {
          callbacks: {
            label: function(context) {
              const label = context.label || '';
              const value = context.parsed;
              const percentage = ((value / totalEducation) * 100).toFixed(1);
              return `${label}: ${value} (${percentage}%)`;
            }
          }
        },
